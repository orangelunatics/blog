...
- 有三个引用，为什么说是单向链表？child、return 是用来构建“树”，“sibling”用来构建同级元素的“单向”链表，通过遍历这个“单向”链表来 dom diff。将虚拟 dom 连接，使得组件更新的流程可以被中断恢复；它把组件渲染的工作分片，到时会主动让出主线程，也就是说将任务的控制权交给浏览器，而 react 需要向浏览器申请时间进行后续的 diff 等操作，这样就不会阻塞布局、动画等周期性任务和交互响应等。
